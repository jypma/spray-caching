...
class ExpiringLruCacheSpec extends Specification with NoTimeConversions {
  ...
  "An LruCache" should {
    ...
    "store uncached values" in { implicit ee: ExecutionEnv =>
    ...
    "return stored values upon cache hit on existing values" in { implicit ee: ExecutionEnv =>
      val cache = lruCache[String]()
      cache(1)("A") must be ("A").await
      cache(1)(failure("Cached expression was evaluated despite a cache hit").asInstanceOf[String]) must be("A").await
      cache.store.toString === "{1=A}"
      cache.size === 1
    }
    "return Futures on uncached values during evaluation and replace these with the value afterwards" in { implicit ee: ExecutionEnv =>
      val cache = lruCache[String]()
      ...
      latch.countDown()
      future1 must be ("A").await
      future2 must be ("A").await
      cache.store.toString === "{1=A}"
      cache.size === 1
    }
    "properly limit capacity" in { implicit ee: ExecutionEnv =>
      val cache = lruCache[String](maxCapacity = 3)
      cache(1)("A") must be ("A").await
      cache(2)(Future.successful("B")) must be ("B").await
      cache(3)("C") must be ("C").await
    ...
    "expire old entries" in { implicit ee: ExecutionEnv =>
    ...
    "refresh an entries expiration time on cache hit" in { implicit ee: ExecutionEnv =>
      val cache = lruCache[String]()
      cache(1)("A") must be ("A").await
      cache(2)("B") must be ("B").await
      cache(3)("C") must be ("C").await
      cache(1)("") must be ("A").await  // refresh
      cache.store.toString === "{1=A, 2=B, 3=C}"
    }
    "be thread-safe" in { implicit ee: ExecutionEnv =>
      ...
      views.transpose must contain(beConsistent).forall
    }
  }
